...
def sammPreprocessing(sel_emotions):
    # 读取配置文件
    ...
    if sel_emotions == 1:
        ...
        EMOTIONS_DICT = eval(config["EMOTIONS_DICT"]["EMOTIONS_DICT_SAMM"])
    SAMM_LOC = config["DATABASE_LOC"]["SAMM_LOC"]
    SAMM_LABEL_LOC = config["DATABASE_LABEL_LOC"]["SAMM_LABEL_LOC"]
    ...
    # 创建受试者编号及绝对路径字典
    sub_dict = {}
    for sub in os.listdir(SAMM_LOC):
        if os.path.isdir(SAMM_LOC + '/' + sub):
            for sample_dir in os.listdir(SAMM_LOC + '/' + sub):
                if os.path.isdir(SAMM_LOC + '/' + sub + '/' + sample_dir):
                    sub_dict[sub + '/' + sample_dir] = SAMM_LOC + '/' + sub + '/' + sample_dir
    ...
    for sub, path in sub_dict.items():
        ...
        if sub_num == -1:
            sub_num = sub.split('/')[0]
            samm_data.append(sub_num.split(" "))
        ...
        onset, apex, offset, label = getSampleInfo(path, SAMM_LABEL_LOC, sel_emotions, EMOTIONS_DICT)
        faces = imageProcessing(path, onset, apex, offset, SAMPLE_NUM, 'SAMM')
        labels = [label] * SAMPLE_NUM
        # 将处理后的图像序列添加至samm_data列表
        sample = [faces, labels]
        samm_data[index].append(sample)
        # print(samm_data[index][0])
        # print(len(samm_data[index][1][0]))
        # print(len(samm_data[index][1][1]))

    print(f"共有{len(samm_data)}个受试者")
    for sub in samm_data:
    ...
    save_data = np.asarray(samm_data, dtype=object)
    np.save(PREPROCESS_SAVE_LOC + '/' + 'SAMM-' + str(sel_emotions) + '-' + str(SAMPLE_NUM), save_data)

    return 'samm-ok'


def getSampleInfo(path, SAMM_LABEL_LOC, sel_emotions, EMOTIONS_DICT):
    # 读取标签excel文件
    table = pd.read_excel(SAMM_LABEL_LOC, header=13)
    # 获取路径中包含的样本编号
    sub_num = path.split('/')[-1]
    # 获取样本编号对应的信息
    cow = table[table['Filename'] == sub_num]
    # 获取起始帧、顶点帧、终止帧和情绪分类
    onset = cow['Onset Frame'].values[0]
    apex = cow['Apex Frame'].values[0]
    offset = cow['Offset Frame'].values[0]
